...
# Data cleaning
dat = HOBO_Rawdata_min13[,c(1,2,3,5,6)] #remain time, temperature, isolation.MJ
rm(HOBO_Rawdata_min13)
...
date_temperature = function(day){
  ...
  nmeanLA1= mean(ndate$`LA1T(DegC)`)                                #calculate the mean
  nmeanLA3= mean(ndate$`LA3T(DegC)`)
  ddate = dclmdat[grep(day,dclmdat$TIMESTAMP),]  
  dmeanLA1=  mean(ddate$`LA1T(DegC)`)
  dmeanLA3=  mean(ddate$`LA3T(DegC)`)
  return(c(dmeanLA1,dmeanLA3,nmeanLA1,nmeanLA3))
}
...
colnames(daily_temperature) = c("DayLA1","DayLA3","NightLA1","NightLA3")
