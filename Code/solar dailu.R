...
sdato = seq(as.Date("2019/9/1"), 
            as.Date("2019/9/30"),"day")  #from start week 1st day to last week 1st day
...
nmeanc = function(sday,eday){
  dater = soldat[grep(sday,soldat$Date),] #date sort from week 1st-7th
...
treatday = matrix(sday,nrow = 1) 

SWave = apply(treatday, 2, nmeanc)
##calculate by row
SWave =c()
SWsd =c()
