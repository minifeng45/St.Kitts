model2.bestsubset = regsubsets(model.2.dataset$AirTC_Avg_HS~.,
                               data.frame(cbind(
                                 model.2.dataset$AirTC_Avg_WS,
                                 model.2.dataset$AirTC_Avg_WS^2,
                                 ...
                                 model.2.dataset$SlrkW_Avg,
                                 model.2.dataset$SlrkW_Avg^2,
                                 model.2.dataset$SlrkW_Avg^3,
                                 model.2.dataset$SlrkW_Avg^4,
                                 model.2.dataset$SlrkW_Avg^5,
                                 model.2.dataset$SlrMJ_Tot,
                                 model.2.dataset$SlrMJ_Tot^2,
                                 model.2.dataset$SlrMJ_Tot^3,
                                 model.2.dataset$SlrMJ_Tot^4,
                                 model.2.dataset$SlrMJ_Tot^5,
                                 model.2.dataset$Rain_mm_Tot,
                                 model.2.dataset$Rain_mm_Tot^2,
                                 model.2.dataset$Rain_mm_Tot^3,
                                 model.2.dataset$Rain_mm_Tot^4,
                                 model.2.dataset$Rain_mm_Tot^5,
                                 ...
                                 model.2.dataset$T_Avg^5)),nvmax =40 , nbest = 3,method = "forward")

reg.summary = summary(model2.bestsubset)
plot(model2.bestsubset, scale = "bic",  xlab = "regressor")
plot(model2.bestsubset, scale = "Cp", xlab = "regressor")
md1 = coef(model2.bestsubset,which(rank(reg.summary$bic) == 1))
md2 = coef(model2.bestsubset,which(rank(reg.summary$bic) == 2))
md3 = coef(model2.bestsubset,which(rank(reg.summary$bic) == 3))
...
model1 <- train(data.frame(cbind(model.2.dataset$AirTC_Avg_WS,
                                 model.2.dataset$AirTC_Avg_WS^3,
                                 model.2.dataset$AirTC_Avg_WS^4,
                                 model.2.dataset$AirTC_Avg_WS^5,
                                 model.2.dataset$RH_WS,
...
model2 <- train(data.frame(cbind(model.2.dataset$AirTC_Avg_WS,
                                 model.2.dataset$AirTC_Avg_WS^2,
                                 model.2.dataset$AirTC_Avg_WS^4,
                                 model.2.dataset$AirTC_Avg_WS^5,
                                 model.2.dataset$RH_WS,
...
model3 <- train(data.frame(cbind(model.2.dataset$AirTC_Avg_WS,
                                 model.2.dataset$AirTC_Avg_WS^3,
                                 model.2.dataset$AirTC_Avg_WS^4,
                                 model.2.dataset$AirTC_Avg_WS^5,
                                 model.2.dataset$RH_WS,
...
library(ggplot2)
plot.dataset = rbind(
  data.frame(Time = as.POSIXct(model.2.dataset$TIMESTAMP), Temperature = model.2.dataset$AirTC_Avg_WS, Category = "AWS"),
  data.frame(Time = as.POSIXct(model.2.dataset$TIMESTAMP), Temperature = actual, Category = "Actual(HS)"),
  data.frame(Time = as.POSIXct(model.2.dataset$TIMESTAMP), Temperature = predict, Category = "Predict")
)

plot.dataset.0202 = plot.dataset %>%
  filter(month(Time) == 2, day(Time) == 2)

p.model.insect = ggplot(plot.dataset.0202, aes(x = Time, y = Temperature, color = Category))+
  geom_line() + 
